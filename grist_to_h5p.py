...
# Sauvegarder temporairement le content.json
temp_content_path = 'content.json'
...
# Manipulation du fichier H5P
source_h5p = 'csti.h5p'

if not os.path.exists(source_h5p):
    print(f"⚠ Fichier {source_h5p} introuvable, seul content.json a été généré")
else:
    # 1) Copier csti.h5p en csti.zip
    temp_zip = 'csti.zip'
    shutil.copy(source_h5p, temp_zip)
    print(f"✓ {source_h5p} copié en {temp_zip}")
    
    # 2) Remplacer content/content.json dans le zip
    with zipfile.ZipFile(temp_zip, 'a') as zf:
        # Supprimer l'ancien content/content.json s'il existe
        files_in_zip = zf.namelist()
        if 'content/content.json' in files_in_zip:
            # On doit recréer le zip sans ce fichier
            temp_zip_new = 'csti_temp.zip'
            with zipfile.ZipFile(temp_zip_new, 'w', zipfile.ZIP_DEFLATED) as new_zf:
                for item in files_in_zip:
                    if item != 'content/content.json':
                        new_zf.writestr(item, zf.read(item))
            # Remplacer l'ancien zip
            os.remove(temp_zip)
            os.rename(temp_zip_new, temp_zip)
        
        # Ajouter le nouveau content.json
        with zipfile.ZipFile(temp_zip, 'a', zipfile.ZIP_DEFLATED) as new_zf:
            new_zf.write(temp_content_path, 'content/content.json')
    
    print(f"✓ content/content.json remplacé dans le zip")
    
    # 3) Renommer avec timestamp
    timestamp = datetime.now().strftime('%d%m%y-%H%M%S')
    final_h5p = f'csti-{timestamp}.h5p'
    os.rename(temp_zip, final_h5p)
    
    print(f"✓ Fichier final créé : {final_h5p}")
